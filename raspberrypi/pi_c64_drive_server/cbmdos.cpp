...
CBMDOSChannel dosSendError()
{
    ...
    // test
    char temp[128];
    sprintf(temp, "%02i,%s,%02i,%02i\r", dosError, dosErrorStrings[i].str, dosTrackError, dosSectorError);
    printf("dos error string: %s\n", temp);
...
CBMDOSChannel dosOpenFile(const char *path, int channel)
{
    ...
    filespec = (char*)alloca(strlen(path) + 5);
...
void dosHandleIO(dataPacket* pkt)
{
