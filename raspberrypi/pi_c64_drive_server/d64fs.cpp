...
int d64MountDisk(CBMDriveData *data, const char *filename)
{
  ...
  if (!(file = fopen(filename, "r")) &&
    ...
    altname = (char*)alloca(strlen(filename) + 5);
...
CBMDOSChannel d64OpenFile(CBMDriveData *data, const char *path, const char *mode)
{
  ...
  entry = d64FindFile((char*)data->image, path);
  if (entry)
    d64BufferFile((char*)data->image, entry, &aChan.buffer, &aChan.length);
