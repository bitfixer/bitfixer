...
// 04.2006 aralbrec
//
// As in ex3b.c but we remove the background clear flag
// from the occluding moving sprite.  We saw what happens
...
main()
{
   ...
   for (i=0; i!=10; i++) {
      ...
      if (!i)
      {
         s = sprtbl[i].s = sp1_CreateSpr(SP1_DRAW_MASK2LB, SP1_TYPE_2BYTE | SP1_TYPE_OCCLUDE, 3, 0, i);
         sp1_AddColSpr(s, SP1_DRAW_MASK2, SP1_TYPE_OCCLUDE, 48, i);
         sp1_AddColSpr(s, SP1_DRAW_MASK2RB, SP1_TYPE_OCCLUDE, 0, i);
