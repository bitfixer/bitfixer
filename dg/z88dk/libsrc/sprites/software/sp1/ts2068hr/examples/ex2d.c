...
// 04.2006 aralbrec
//
// A second look at LOAD-type sprites as in ex2c.c but this
// time we make the left and right borders LBIM/RBIM type
// (load type with implied mask on left/right border).  The
// implied mask is used to preserve the underlying graphics
// on the left and right sides of the sprite as the sprite
// is rotated horizontally.
/////////////////////////////////////////////////////////////

// zcc +ts2068 -vn ex2d.c -o ex2d.bin -create-app -lsp1 -lmalloc
...
main()
{
   ...
   for (i=0; i!=10; i++) {
      ...
      s = sprtbl[i].s = sp1_CreateSpr(SP1_DRAW_LOAD2LBIM, SP1_TYPE_2BYTE, 3, 0, i);
      sp1_AddColSpr(s, SP1_DRAW_LOAD2, 0, 48, i);
      sp1_AddColSpr(s, SP1_DRAW_LOAD2RBIM, 0, 0, i);
