...
The RCS variants of the decompression routines (dzx7_smart_rcs() and dzx_agile_rcs())
pre-mangle zx spectrum screen$ to make picture data linear in memory prior
to compression.  This only makes sense for screen$ data originating from the
zx spectrum target but the routines have been retained for all targets so
that data compressed in this manner can still be decompressed on any machine.
